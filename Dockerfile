...
COPY --from=node-builder /root/.cache/node /home/zoobooru/.cache/node
...
ARG HOST_UID=1000
ARG HOST_GID=1000
RUN addgroup --gid ${HOST_GID} zoobooru && \
  adduser -S --shell /bin/sh --uid ${HOST_UID} zoobooru && \
  addgroup zoobooru wheel && \
  echo "zoobooru ALL=(ALL) NOPASSWD:ALL" >> /etc/sudoers
